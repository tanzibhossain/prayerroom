...
class CustomerController extends Controller
{
    ...
    public function listing_add() {
        ...
        if($detail == null) {
            ...
            if($remaining_listing == 0) {
                return Redirect()->route('customer_package')->with('error', ERR_LISTING_MAXIMUM_LIMIT_REACHED);
            }
    ...
    public function free_enroll($id) {
        ...
        // First check if this user has already enrolled free package
        $check = PackagePurchase::where('user_id',$user_data->id)->where('paid_amount',0)->first();
        if($check) {
            return Redirect()->route('customer_package')->with('error', ERR_ALREADY_ENROLLED_FREE);
        }
    ...
    public function buy_package($id) 
    {
        $g_setting = GeneralSetting::where('id',1)->first();
        $package_detail = Package::where('id',$id)->first();
        return view('front.customer_package_buy', compact('g_setting', 'package_detail'));
    }
    ...
    public function paypal_success(Request $request)
    {
        ...
        {   
            $data['currently_active'] = 0;
            PackagePurchase::where('user_id',Auth::guard('web')->user()->id)->update($data);
            ...
            $final_price = round(session()->get('package_price'),2);
            $obj = new PackagePurchase;
            ...
            $message = str_replace('[[customer_name]]', Auth::guard('web')->user()->name, $message);
            $message = str_replace('[[transaction_id]]', $trxid, $message);
            $message = str_replace('[[payment_method]]', $payment_method, $message);
            $message = str_replace('[[paid_amount]]', '$'.$final_price, $message);
            ...
            Mail::to(Auth::guard('web')->user()->email)->send(new Websitemail($subject,$message));

            unset($_SESSION['package_id']);
            unset($_SESSION['package_name']);
            unset($_SESSION['package_price']);

            return redirect()->route('customer_dashboard')->with('success', SUCCESS_PACKAGE_PURCHASE);
    ...
    {
        return redirect()->route('customer_dashboard')->with('error', ERR_PAYMENT_CANCELLED);
    }
    ...
    public function bank(Request $request)
    {
        ...
        $final_price = $request->package_price;
        $final_price = round($final_price,2);

        if(env('PROJECT_MODE') == 0) {
            return Redirect()->route('customer_package_purchase_history')->with('error', env('PROJECT_NOTIFICATION'));
        } else {
            $package_detail = Package::where('id',$request->package_id)->first();
            $valid_days = $package_detail->valid_days;
            $start_date = date('Y-m-d');
            $end_date = date('Y-m-d', strtotime("+$valid_days days"));
